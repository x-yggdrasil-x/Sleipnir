...
CzODINTracker::CzODINTracker(std::string strWalletFile)
{
...
CzODINTracker::~CzODINTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzODINTracker::Init()
{
...
bool CzODINTracker::Archive(CMintMeta& meta)
{
...
bool CzODINTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzODINTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzODINTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzODINTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzODINTracker::GetSerialHashes()
{
...
CAmount CzODINTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzODINTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzODINTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzODINTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzODINTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzODINTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzODINTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzODINTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzODINTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzODINTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzODINTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzODINTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzODINTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzODINTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzODINTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzODINTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzODINTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzodin from DB\n", __func__, listDeterministicDB.size());
    }

                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->labelzODINSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zODINpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzODIN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zODIN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zODIN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zODIN in ") +
...
void PrivacyDialog::on_pushButtonSpendzODIN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zODIN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zODIN
        sendzODIN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zODIN
    sendzODIN();
}

void PrivacyDialog::on_pushButtonZODINControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZODINControlDialog* zODINControl = new ZODINControlDialog(this);
    zODINControl->setModel(walletModel);
    zODINControl->exec();
}

void PrivacyDialog::setZODINControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzODINSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzODIN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ODIN Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zODINpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zODIN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ODIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zODIN</b>";
    ...
    // use mints from zODIN selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZODINControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZODINControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zODIN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zODIN"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZODIN_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zODIN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zODIN"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zODIN was spent successfully update the addressbook with the label
    ...
    // Clear zodin selector in case it was used
    ZODINControlDialog::setSelectedMints.clear();
    ui->labelzODINSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zODIN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ODIN, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zODIN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zodinTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zODIN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zODIN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zODIN "));
    ui->labelzODINAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzODINSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzODIN->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zODIN
        ui->pushButtonMintzODIN->setEnabled(false);
        ui->pushButtonMintzODIN->setToolTip(tr("zODIN is currently disabled due to maintenance."));

        // Spend zODIN
        ui->pushButtonSpendzODIN->setEnabled(false);
        ui->pushButtonSpendzODIN->setToolTip(tr("zODIN is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zODIN
        ui->pushButtonMintzODIN->setEnabled(true);
        ui->pushButtonMintzODIN->setToolTip(tr("PrivacyDialog", "Enter an amount of ODIN to convert to zODIN", 0));

        // Spend zODIN
        ui->pushButtonSpendzODIN->setEnabled(true);
        ui->pushButtonSpendzODIN->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

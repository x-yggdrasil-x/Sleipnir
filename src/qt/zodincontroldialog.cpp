...
#include "ui_zodincontroldialog.h"
...
std::set<std::string> ZODINControlDialog::setSelectedMints;
std::set<CMintMeta> ZODINControlDialog::setMints;

ZODINControlDialog::ZODINControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZODINControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZODINControlDialog::~ZODINControlDialog()
{
    delete ui;
}

void ZODINControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZODINControlDialog::updateList()
{
...
void ZODINControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZODINControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZODIN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZODINControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZODINControlDialog::GetSelectedMints()
{
...
void ZODINControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

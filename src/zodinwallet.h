...
#ifndef ODIN_ZODINWALLET_H
#define ODIN_ZODINWALLET_H
...
public:
    CzODINWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZODIN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZODIN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ODIN_ZODINWALLET_H

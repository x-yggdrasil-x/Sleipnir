...
CzODINWallet::CzODINWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zodin seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZODINSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZODINSeed_deprecated()) {
                    LogPrintf("%s: Updated zODIN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zodin seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zODIN wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzODINWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZODINCount(nCountLastUsed);
    else if (!walletdb.ReadZODINCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzODINWallet::Lock()
{
    seedMaster = 0;
}

void CzODINWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzODINWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZODIN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzODINWallet::LoadMintPoolFromDB()
{
...
void CzODINWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzODINWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzODINWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zodinTracker->HasPubcoinHash(pMint.first)) {
...
bool CzODINWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZODIN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zodinTracker which also adds to database
    pwalletMain->zodinTracker->Add(dMint, true);
...
void CzODINWallet::SeedToZODIN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzODINWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzODINWallet::GenerateDeterministicZODIN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzODINWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzODINWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

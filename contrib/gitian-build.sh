...
VERSION=
commit=false
url=https://github.com/odinblockchain/odin
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the odin, gitian-builder, gitian.sigs, and odin-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/odinblockchain/odin
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit odin=${COMMIT} --url odin=${url} ../odin/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../odin/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/odin-*.tar.gz build/out/src/odin-*.tar.gz ../odin-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit odin=${COMMIT} --url odin=${url} ../odin/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/odin-*.zip build/out/odin-*.exe ../odin-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit odin=${COMMIT} --url odin=${url} ../odin/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/odin-*.tar.gz build/out/odin-*.dmg ../odin-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit odin=${COMMIT} --url odin=${url} ../odin/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../odin/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/odin-*.tar.gz build/out/src/odin-*.tar.gz ../odin-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../odin/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/odin-*win64-setup.exe ../odin-binaries/${VERSION}
	    mv build/out/odin-*win32-setup.exe ../odin-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../odin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/odin-osx-signed.dmg ../odin-binaries/${VERSION}/odin-${VERSION}-osx.dmg
	fi
	popd || exit
